...
{
    public abstract class Auth0ClientBase
    {
        #region Private Members
        private static readonly ILog Logger = LoggerSource.Instance.GetLogger(typeof(Auth0ClientBase));
        ...
        protected Auth0ClientBase(int portalId, AuthMode mode, string service)
        {
            AuthTokenExpiry = new TimeSpan(14, 0, 0, 0);
            Service = service;

            APIKey = Auth0ConfigBase.GetConfig(Service, portalId).APIKey;
            APISecret = Auth0ConfigBase.GetConfig(Service, portalId).APISecret;
            Mode = mode;
        ...
        protected Uri UserEndpoint { get; set; }
        ...
        private void ExchangeCodeForToken()
        {
            ...
            {
                { OAuthClientIdKey, APIKey },
                { OAuthRedirectUriKey, CallbackUri.ToString() },
                { OAuthClientSecretKey, APISecret.ToString() },
                { OAuthGrantTyepKey, "authorization_code" },
                { OAuthCodeKey, VerificationCode }
            };

            //DNN-6265 Support for OAuth V2 optional parameter
            if (!String.IsNullOrEmpty(APIResource))
            {
                parameters.Add("resource", APIResource);
            }


            string responseText = ExecuteWebRequest(HttpMethod.POST, TokenEndpoint, Json.Serialize(parameters), String.Empty);
        ...
        public virtual TUserData GetCurrentUser<TUserData>() where TUserData : UserData
        {
            ...
            string responseText = ExecuteWebRequest(HttpMethod.GET, GenerateRequestUri(UserEndpoint.ToString(), accessToken), null, String.Empty);
